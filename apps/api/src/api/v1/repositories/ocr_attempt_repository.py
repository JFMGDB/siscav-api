...
class OcrAttemptRepository:
    @staticmethod
    def create(db: Session, *, success: bool) -> OcrAttempt:
        row = OcrAttempt(timestamp=datetime.now(UTC), success=success)
        db.add(row)
        try:
            db.commit()
            db.refresh(row)
    ...
    @staticmethod
    def get_daily_success_rate(db: Session, day: date) -> tuple[int, float]:
