...
Em desenvolvimento, a API carrega automaticamente `.env` e `.env.local` na raiz do repositório. Copie `env.local.example` para `.env.local` ou defina variáveis no IDE; `source .env.local` é opcional.
