...
A interface para **ligar câmara por USB ou por URL (Wi‑Fi / IP)** e ver **vídeo em tempo real** é implementada num **repositório frontend separado** com **Next.js** e **TypeScript**. O fluxo de vídeo é sobretudo **no browser** (por exemplo `getUserMedia` para USB, ou `<img>`/`<video>` para streams MJPEG/HLS); **não** depende de endpoints de streaming nesta API.
...
- Documentação técnica: `docs/api/technical-documentation.md`
