...
**Sources:** `.planning/codebase/CONCERNS.md`, repository scan, architecture review.
...
| ID | Resolution |
|----|------------|
| BUG-002 | Refresh token API exists at `POST /api/v1/login/refresh-token` |
| BUG-009 | Deprecated `crud/` package removed from codebase |
| BUG-010 | No `docker-compose.yml` in repo — intentional; local Postgres via Docker run command documented in setup guides |
