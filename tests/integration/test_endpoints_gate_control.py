...
class TestGateControlEndpoints:
    ...
    def test_close_gate_admin_simulated(self, client: TestClient, admin_auth_token: str):
        get_settings.cache_clear()
        response = client.post(
            "/api/v1/gate_control/close",
            headers={"Authorization": f"Bearer {admin_auth_token}"},
        )
    ...
    @patch("apps.api.src.api.v1.controllers.gate_controller.urlopen")
    def test_close_gate_live_upstream_2xx(
        self,
        mock_urlopen: MagicMock,
        client: TestClient,
        admin_auth_token: str,
        ...
        try:
            ...
            assert call_req.full_url == "http://actuator.test/close"
        finally:
            monkeypatch.delenv("GATE_ACTUATOR_URL", raising=False)
            get_settings.cache_clear()
