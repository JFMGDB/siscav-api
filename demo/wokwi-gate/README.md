...
Visual feedback: green LED ON + servo moves to 90° + `GATE OPEN` on Serial Monitor.
...
4. Replace the diagram with [`diagram.json`](diagram.json).
...
Expected: Serial `GATE OPEN`, servo at 90°, LED on, HTTP body `{"ok":true}`.
